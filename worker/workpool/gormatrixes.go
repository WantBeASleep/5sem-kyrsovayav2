...
func plusOp(x, y *structs.Matrix, i, j int) int {
	return x.Grid[i][j] + y.Grid[i][j]
}

func multOp(x, y *structs.Matrix, i, j int) int {
	res := 0
...
func GetMatrixCalcTasks(taskQ chan task, opResponseChannel chan structs.Matrix, x, y structs.Matrix, op token.Token) {
	switch op {
	case token.ADD:

		//Create new matrix
		res := structs.Matrix{
			Size: structs.MatrixSize{
				Rows: x.Size.Rows,
				Cols: x.Size.Cols,
			},
